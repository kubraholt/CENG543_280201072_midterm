...
echo ""
echo "Configuration:"
echo "  Corpus: HotpotQA"
echo "  Sparse Retrieval: BM25"
...
echo "[Step 1/10] Preparing HotpotQA dataset..."
...
echo "[Step 2/10] Building BM25 retriever..."
python 2_build_bm25_retriever.py
if [ $? -ne 0 ]; then
    echo "Error in BM25 indexing. Exiting."
    exit 1
fi
echo ""
...
echo "[Step 6/10] Running BM25 + FLAN-T5-base RAG pipeline..."
python 6_rag_bm25.py
if [ $? -ne 0 ]; then
    echo "Error in BM25 RAG pipeline. Exiting."
    exit 1
fi
echo ""
...
echo "  - models/                 (BM25 & SBERT indexes)"
