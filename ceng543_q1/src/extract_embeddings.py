...
from data_utils import get_imdb_splits, IMDBTextDataset, collate_glove, collate_bert
from glove_model import BiRNNClassifier
from bert_model import DistilBertRNN
from torch.utils.data import DataLoader
from gensim.models import KeyedVectors

def load_glove_model(path, device, rnn_type, hidden_dim=128):
    model = BiRNNClassifier(input_dim=300, hidden_dim=hidden_dim, rnn_type=rnn_type, num_layers=2)
    model.load_state_dict(torch.load(path, map_location=device))
    model.to(device).eval()
    return model

def load_bert_model(path, device, rnn_type, hidden_dim=128):
    model = DistilBertRNN(
...
def main(args):
    ...
    train_texts, train_labels, test_texts, test_labels = get_imdb_splits()
    ...
    if args.mode == 'glove':
        ...
        ds = IMDBTextDataset(test_texts, test_labels, glove_vectors=glove, mode='glove')
        dl = DataLoader(ds, batch_size=args.batch_size, shuffle=False, collate_fn=collate_glove)
        model = load_glove_model(args.model_path, device, rnn_type=args.rnn_type, hidden_dim=args.hidden_dim)
        embeddings, labels = extract_glove_embeddings(model, dl, device)
    else:
        from transformers import DistilBertTokenizerFast
        tokenizer = DistilBertTokenizerFast.from_pretrained("distilbert-base-uncased")
        ds = IMDBTextDataset(test_texts, test_labels, tokenizer=tokenizer, mode='bert', max_len=args.max_len)
        dl = DataLoader(ds, batch_size=args.batch_size, shuffle=False, collate_fn=collate_bert)
        model = load_bert_model(args.model_path, device, rnn_type=args.rnn_type, hidden_dim=args.hidden_dim)
