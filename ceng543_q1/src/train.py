...
def main(args):
    ...
    if args.mode == 'glove':
        ...
        train_ds = IMDBTextDataset(train_texts, train_labels, glove_vectors=glove, mode='glove')
        test_ds = IMDBTextDataset(test_texts, test_labels, glove_vectors=glove, mode='glove')
        ...
        train_ds = IMDBTextDataset(train_texts, train_labels, tokenizer=tokenizer, mode='bert', max_len=args.max_len)
        test_ds = IMDBTextDataset(test_texts, test_labels, tokenizer=tokenizer, mode='bert', max_len=args.max_len)
        ...
        model = DistilBertRNN(bert_model_name="distilbert-base-uncased", hidden_dim=args.hidden_dim,
                              rnn_type=args.rnn_type, num_layers=args.num_layers, freeze_bert=args.freeze_bert).to(device)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), os.path.join(args.out_dir, f"best_{args.mode}_{args.rnn_type}.pt"))
