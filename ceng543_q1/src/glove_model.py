...
class BiRNNClassifier(nn.Module):
    def __init__(self, input_dim=300, hidden_dim=128, num_layers=1, rnn_type='lstm', num_classes=2, dropout=0.2):
        ...
        if self.rnn_type == 'lstm':
            self.rnn = nn.LSTM(input_dim, hidden_dim, num_layers=num_layers, bidirectional=True, batch_first=True, dropout=dropout)
        else:
            self.rnn = nn.GRU(input_dim, hidden_dim, num_layers=num_layers, bidirectional=True, batch_first=True, dropout=dropout)
